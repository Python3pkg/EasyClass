...
class EasyClassMeta(type):
    def __new__(cls, name, bases, attrs):
        ...
        for attribute, val in list(attributes.items()):
...
class EasyClass(with_metaclass(EasyClassMeta, object)):
    ...
    def __init__(self, **kwargs):
        defined_attributes = [x for x, y in list(vars(self.__class__).items()) if issubclass(y.__class__, Variable)]
    ...
    def check_nullable(self):
        non_null_attributes = [
            key for key, value in list(vars(self.__class__).items())
